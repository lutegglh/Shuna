...
class GoogleDriveHelper:
    ...
    def __upload_empty_file(self, path, file_name, mime_type, parent_id=None):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'Galuh Angga Saputra',
            'mimeType': mime_type,
        }
    ...
    def upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        file_metadata = {
            'name': file_name,
            'description': 'https://t.me/mirror_gam',
            'mimeType': mime_type,
        }
        try:
    ...
    def clone(self, link):
        ...
        try:
            ...
            if meta.get("mimeType") == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'\n<b>🗃️ SubFolder: </b><code>{self.total_folders}</code>'
                msg += f'\n<b>📁 File: </b><code>{self.total_files}</code>'
                buttons = button_build.ButtonMaker()
                if SHORTENER is not None and SHORTENER_API is not None:
                    surl = short_url(durl)
                    buttons.buildbutton("☁️ G-Drive Link", surl)
                else:
                    buttons.buildbutton("☁️ G-Drive Link", durl)
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False):
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if not response["files"] and noMulti:
                ...
                continue
            if not Title:
                msg += f'<h4>🔎 Ditemukan Hasil Dari: {fileName}</h4><br><br>'
                Title = True
            ...
            for file in response.get('files', []):
                if file.get('mimeType') == "application/vnd.google-apps.folder":
                    ...
                    if SHORTENER is not None and SHORTENER_API is not None:
                        ...
                        msg += f"<b><a href={sfurl}>☁️ G-Drive Link</a></b>"
                    else:
        ...
        msg = f"<b>🔥 Ditemukan <code>{all_contents_count}</code>\n🔎 Hasil Dari <code>{fileName}</code></b>"
        buttons = button_build.ButtonMaker()
